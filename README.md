...
| Persistent sessions | Saves cross-project sessions, live workers, activities, model state, terminals, branches, goals, editable titles, pins, archives, title search, and URL-addressable web navigation. | Adds content search, filters, sections, manual ordering, pagination, and session forks. | CodeCrab covers persistent metadata, title discovery, and concurrent work; Codex provides richer large-catalog organization. |
...
The terminal `/sessions` view and web sidebar both expose the complete project/session hierarchy, including the top-level **No project** group. Both can search session titles case-insensitively across every registered project, including archived sessions; stronger title matches appear first and selecting one resumes its exact project context. Press `/` in the terminal session picker to search, or use the web sidebar search field; clearing the query restores the hierarchy. Search is intentionally title-only and never scans conversation content. Child sessions are indented recursively; parents use disclosure chevrons and show the number of hidden descendants when collapsed. In the terminal, left/right collapse and expand projects or session branches, `R` renames, `P` toggles a pin, and `A` archives or restores the selected session. The web edits a title directly and keeps pin and archive buttons visible on every session row. A manual title is permanent, trimmed, non-empty, limited to 120 characters, and unaffected by later edits to the first message.
...
| `GET` | `/api/sessions/search?query=...` | Search session titles across all registered projects |
